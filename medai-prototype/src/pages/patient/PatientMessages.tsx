...
export default function PatientMessages() {
  ...
  const myThreads = messageThreads.filter((t) => t.participants.includes(patient.id))
  ...
  const send = async () => {
    ...
    await api.sendMessage({
      threadId: active.id,
      fromId: patient.id,
      fromName: patient.name,
      toId: active.participants.find((p) => p !== patient.id) ?? 'doc_1',
      body: draft.trim(),
    })
    setLocalMessages((prev) => [
      ...prev,
      { id: `m_${Date.now()}`, threadId: active.id, fromId: patient.id, fromName: patient.name, toId: active.participants.find((p) => p !== patient.id) ?? 'doc_1', body: draft.trim(), attachments: [], sentAt: new Date().toISOString(), read: false },
    ])
    setDraft('')
    setSending(false)
  }
  ...
  return (
    <div>
      <PageHeader title="Messages" subtitle="Secure, encrypted conversations with your care team." />

      {myThreads.length === 0 ? (
        <EmptyState icon={MessageSquare} title="No conversations" description="Reach out to your care team to start a secure conversation." />
      ) : (
              ...
              {myThreads.map((t) => (
                <button key={t.id} onClick={() => setActiveId(t.id)} className={cn('flex w-full items-start gap-3 rounded-xl p-3 text-left transition-colors', activeId === t.id ? 'bg-primary/5' : 'hover:bg-muted')}>
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-primary/10 text-sm font-bold text-primary">
                    {(t.participantNames[0] ?? 'Team').replace('Dr. ', '').split(' ').map((s) => s[0]).join('')}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className="truncate text-sm font-semibold">{t.participantNames[0] ?? 'Care team'}</p>
          ...
          <Card className="flex flex-col overflow-hidden lg:col-span-2">
            <div className="flex items-center gap-3 border-b p-4">
              <div className="grid h-9 w-9 place-items-center rounded-full bg-primary/10 text-sm font-bold text-primary">
                {(active?.participantNames[0] ?? 'T').replace('Dr. ', '').split(' ').map((s) => s[0]).join('')}
              </div>
              <div>
                <p className="text-sm font-semibold">{active?.participantNames[0] ?? 'Care team'}</p>
                <p className="flex items-center gap-1.5 text-[11px] text-success"><span className="h-1.5 w-1.5 rounded-full bg-success" /> Online</p>
              </div>
            </div>

            <div className="flex-1 space-y-3 overflow-y-auto p-4">
              {threadMessages.map((m) => {
                const mine = m.fromId === patient.id
                return (
